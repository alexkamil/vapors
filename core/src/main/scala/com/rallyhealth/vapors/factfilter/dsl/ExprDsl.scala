...
object ExprDsl extends ExprBuilderSyntax with ExprBuilderCatsInstances {
  ...
  final class WhenThenBuilder[F[_], V, R, P](
    ...
    def elseReturn(
      ...
      Expr.When(condExpr, thenExpr, elseExpr, captureResult)
  }
  ...
  final class WithFactsOfTypeBuilder[T, P](factTypeSet: FactTypeSet[T])(implicit captureInput: CaptureFromFacts[T, P]) {
    ...
    def returnInput(
      implicit
      captureResult: CaptureRootExpr[TypedFactSet[T], P],
    ): Expr.WithFactsOfType[T, TypedFactSet[T], P] =
      Expr.WithFactsOfType(factTypeSet, input[Set, TypedFact[T], P](captureInput), captureResult)
  }
